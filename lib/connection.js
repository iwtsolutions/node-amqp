...
Connection.prototype.addAllListeners = function() {
  ...
  self.addListener('data', function (data) {
    if(self.parser != null){
      try {
        ...
        self.emit('amqp_error', exception);
        return;
      }
    }
  ...
  self.addListener('amqp_error', function backoff(e) {
    self.emit('error', e);
    ...
    if (!self.connectionAttemptScheduled) {
      ...
      if (self.implOptions.reconnect) {
        ...
        self.removeListener('amqp_error', backoff);
      }
    }
  });
  ...
  self.addListener('end', function (){
    if (!this.readyEmitted){
      this.emit('amqp_error', new Error(
  ...
  if (this.options.heartbeat) {
    ...
    this._inboundHeartbeatTimer = setTimeout(function () {
      if(self.socket.readable)
        self.emit('amqp_error', new Error('no heartbeat or data in last ' + gracePeriod + ' seconds'));
    }, gracePeriod * 1000);
  }
};
  ...
  switch (method) {
    ...
    case methods.connectionStart:
      ...
      if (args.versionMajor !== 0 && args.versionMinor != 9) {
        this.socket.end();
        this.emit('amqp_error', new Error("Bad server version"));
        return;
      }
...
Connection.prototype._createSocket = function() {
  ...
  var events = ['close', 'connect', 'data', 'drain', 'amqp_error', 'end', 'secureConnect', 'timeout'];
